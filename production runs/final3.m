%FINAL3 - Production run for ejection-collision connections to P1 at equal masses for L4 energy.
...
unstableFileID = [savePath, 'P1_unstable_', runID]; % filename to save unstable manifold atlas data
stableFileID = [savePath, 'P1_stable_', runID]; % filename to save stable manifold atlas data
collisionFileID = [savePath, 'P1_P1_connections_', runID]; % filename to save collision data
%% ================================================== SET RUN PARAMETERS  ==================================================
...
%%  ================================================== GROW OR LOAD UNSTABLE MANIFOLD FOR P1  ==================================================
if isfile(unstableFileID) % check if unstable manifold is computed already
    ...
    initialData = setreginitialdata(mu, 1, N, initialUnstableArc, false); % set up some initial data on a F1 circle
    
    % Integrate the collision manifold for the large primary backwards in time.
    unstableBd = RegCRTBPChart(initialData, 'Taylor', 0, truncation, [mu, C], 1, 'InitialScaling', tauGuess, 'boundary', true);
    bdCheck = @(obj, boundaryChart, maxTau)boundarycheck(obj, boundaryChart, maxTau, 'SubDivideParameter', subDivideParameter);
    atlasP1F = Atlas(unstableBd, tauGuess, bdCheck, @advectioncheck, 'MaxTau', maxTau);
    while ~isempty(atlasP1F.LeafStack)
        fprintf('%0.4f \n', min([atlasP1F.LeafStack.TimeSpan]))
        atlasP1F.growboundary('RegTime', @regtime)
    end
    save(stableFileID, 'unstableBd', 'atlasP1F') % save manifold data
end
...
if isfile(collisionFileID) % check if collisions have already been computed
    ...
    load(unstableFileID)
    atlasF = atlasP1F; % set (unstable) forward time atlas variable
